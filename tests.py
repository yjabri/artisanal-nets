...
class Tests(unittest.TestCase):
    ...
    def test_two_layer_sigmoid_backpropagation_with_regularization(self):
        ...
        _, cache = nn.forward_pass(X, params, activations)
        grads = nn.backpropagation(X, Y, params, cache, dactivations)
        grads = nn.add_l2_regularization_to_backpropagation(params,
                                                            grads,
                                                            lambda_,
                                                            m)
        ...
        def fn(flattened_params):
            params = unflatten_params(flattened_params, params_meta)
            predictions, _ = nn.forward_pass(X, params, activations)
            cost = nn.sigmoid_cross_entropy(Y, predictions)
            cost = nn.add_l2_regularization_to_cross_entropy(cost,
                                                             params,
                                                             lambda_,
                                                             m)
            return cost
    ...
    def test_two_layer_softmax_backpropagation_with_regularization(self):
        m = 10
        X = np.random.randn(4, m)
        lambda_ = 0.01
        ...
        def fn(flattened_params):
            params = unflatten_params(flattened_params, params_meta)
            predictions, _ = nn.forward_pass(X, params, activations)
            cost = nn.softmax_cross_entropy(Y, predictions)
            cost = nn.add_l2_regularization_to_cross_entropy(cost,
                                                             params,
                                                             lambda_,
                                                             m)
